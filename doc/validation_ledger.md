...
**Summary:** **18 / 20 at `reproducerOnly` base, 2 / 20 at `runtimeVerified` base, 0 / 20 at `unvalidated`**.
...
count: 15 family-severity pairs across 12 unique stableIds** —
`NetworkMonitor.{slow_request {warning + critical}, large_response, request_frequency}`
(v0.19.14 adds the critical-tier bracket via `additionalBrackets[2]`,
second tier-stack raise; backfills `observedAxisArgKey: 'observedDurationMs'`
on the canonical warning bracket),
`MemoryPressure.heap_growing`, `FrameTiming.jank_detected`,
`RebuildDetector.rebuild_activity {warning + critical}` (warning via
`perStableIdTier` v0.19.12; critical via `additionalBrackets[0]`
v0.19.21 — third tier-stack raise),
`HeavyCompute.heavy_compute {warning + critical}` (single-family base raise;
v0.19.13 adds critical-tier bracket — first tier-stack raise),
`PlatformChannel.platform_channel_traffic` (single-family base raise),
`RepaintDetector.excessive_repaint` (warning via `perStableIdTier`
v0.21.0; `excessive_repaint_debug` and parametric
`repaint_debug_<typeName>` stay base reproducerOnly),
`StreamResourceDetector.stream_resource_growth` (warning via
`perStableIdTier` v0.26.0; single-family detector — no critical tier),
`TrackedResourceDetector.tracked_resource_concurrent` (warning via
`perStableIdTier` v0.29.0; `PerformanceIssue.captureTraceStableId`
routes parametric emissions through bare family for byte-exact bracket
matching),
`TrackedResourceDetector.tracked_resource_long_lived` (warning via
`additionalBrackets[0]` v0.30.0 — fourth tier-stack raise; separate
axis `oldestInstanceAgeSeconds` on the 300 s default threshold;
detector overwrites `longLivedFirstCrossMicros` each sweep so a
long-lived overshoot emits per sweep with monotonically-increasing
age, letting `observedAxisReduction: 'max'` pick the leg-end value).
...
**Evidence not uniform across the 18.** Reproducers ship in two
...
|---|---|---|---|
| Tracked Resource | `reproducerOnly` | [`tracked_resource_reproducer_test.dart`](../test/validation/tracked_resource_reproducer_test.dart) + 6 captures under [`test/validation/captures/tracked_resource_concurrent/`](../test/validation/captures/tracked_resource_concurrent/) and [`test/validation/captures/tracked_resource_long_lived/`](../test/validation/captures/tracked_resource_long_lived/) | Pure-Dart, opt-in. `Sleuth.trackResource(name, resource)` registers; tracker keeps `WeakReference<Object>` + Finalizer token + first-seen timestamp per registration. Two stableIds, both `confirmed`, both **runtimeVerified**: `tracked_resource_concurrent.warning` (live count > `trackedResourceMaxConcurrent`, default 5; **v0.29.0 — bracket threshold 6 `instances`, atTolerance 0.5 (at-band [6, 9]), aboveCeilingMultiplier 3.0 (ceiling 18), `observedAxisArgKey: 'liveInstanceCount'`, iPhone 12 / iOS 17.5 / Flutter 3.41.4 capture triad**) and `tracked_resource_long_lived.warning` (oldest instance alive past `trackedResourceLongLivedSeconds`, default 300 s; **v0.30.0 — `additionalBrackets[0]`, threshold 300 `seconds`, atTolerance 0.5 (at-band [300, 450]), aboveCeilingMultiplier 3.0 (ceiling 900), `observedAxisArgKey: 'oldestInstanceAgeSeconds'`; real-time waits at the 300 s default threshold. Detector overwrites `longLivedFirstCrossMicros = nowMicros` each sweep (was `??=`) so a long-lived overshoot emits per sweep with monotonically-increasing age; the trace contains an ascending-age series and `observedAxisReduction: 'max'` picks the leg-end value naturally**). Periodic sweep (default 10 s) prunes finalised refs + evaluates thresholds. **Capture-mode plumbing**: synchronous `flushConcurrentEvaluation()`; `untrackAll(name)` drops bucket + detaches Finalizers for leg isolation; `resetCaptureState()` (auto-invoked by `Sleuth.markScenarioBegin`) clears per-leg observables + every bucket's `concurrentFirstCrossMicros` / `longLivedFirstCrossMicros`; per-name `lastObservedLiveCountFor(name)` / `peakObservedLiveCountFor(name)` / `lastObservedAgeSecondsFor(name)` / `peakObservedAgeSecondsFor(name)` getters. Parametric emissions (`tracked_resource_concurrent:<name>` / `tracked_resource_long_lived:<name>`) route the capture trace event through `PerformanceIssue.captureTraceStableId` (bare family). LRU cap (`trackedResourceMaxDistinctNames`, default 1000) bounds the bucket map; eviction detaches per-ref Finalizer entries. Cross-isolate registration is a no-op. Primitive / record targets dropped silently via `droppedTargetsCount`. `coveredStableIds = {'tracked_resource_concurrent', 'tracked_resource_long_lived'}`; `coveredThresholds = {'tracked_resource_concurrent.warning'}` (canonical) + `additionalBrackets[0].coveredThresholds = {'tracked_resource_long_lived.warning'}`. CausalGraphRule edges to `heap_growing` for both stableIds. |
| Network Monitor | `reproducerOnly` | [`network_monitor_reproducer_test.dart`](../test/validation/network_monitor_reproducer_test.dart) | Base tier `reproducerOnly` with `perStableIdTier{slow_request: runtimeVerified}` (v0.18.0), `large_response` and `request_frequency` raised via the same mechanism (v0.19.9); `http_error_spike` + `high_frequency_same_path` stay base. Reproducer: Layer 1 `processRecord` boundary tests, Layer 2 loopback `HttpServer` exercising the full `SleuthHttpOverrides` → `_MonitoringHttpClient` → `RequestRecord` → `processRecord` pipeline, Layer 3 covers other families. **slow_request warning** (v0.18.0): canonical bracket — threshold 1000 ms, atTolerance 0.10, aboveCeilingMultiplier 2.0 (above-ceiling 2000 ms clears 3000 ms critical). **slow_request critical** (v0.19.14, `additionalBrackets[2]`, second tier-stack raise): threshold 3000 ms, atTolerance 0.40 (at-band [3000, 4200] — wider re-record headroom for operator-environment RTT variance), aboveCeilingMultiplier 2.0 (ceiling 6000 ms; no super-tier above; iOS NSURLSession 60 s default leaves margin). Both brackets stamp `extraTraceArgs.observedDurationMs` (worst record's `durationMs`), declare `observedAxisArgKey: 'observedDurationMs'`, and tighten `observedAxisTolerance: 0.10` because loopback measurement paths agree to the millisecond. Cross-spec uniqueness key `(stableId, severityLabel, argKey)` distinguishes the warning + critical pair via severityLabel even though they share argKey. **Other family axes**: `large_response.warning` (1 MiB, additionalBrackets[0], `observedAxisArgKey: 'observedResponseBytes'`); `request_frequency.warning` (30 events / 5 s window, additionalBrackets[1], `observedAxisArgKey: 'observedRequestCount'`, atTolerance 0.50 absorbs iOS scheduling jitter). Six on-device slow_request captures (iPhone 12 / iOS 17.5 / Flutter 3.41.x): warning [`slow_request_below.json`](../test/validation/captures/network_monitor/slow_request_below.json), [`slow_request_at.json`](../test/validation/captures/network_monitor/slow_request_at.json), [`slow_request_above.json`](../test/validation/captures/network_monitor/slow_request_above.json); critical [`slow_request_critical_below.json`](../test/validation/captures/network_monitor/slow_request_critical_below.json), [`slow_request_critical_at.json`](../test/validation/captures/network_monitor/slow_request_critical_at.json), [`slow_request_critical_above.json`](../test/validation/captures/network_monitor/slow_request_critical_above.json). Critical-below fires `.warning` (not `.critical`); schema's name-scoped no-record check accepts. `coveredStableIds = {slow_request, large_response, request_frequency, http_error_spike, high_frequency_same_path}`, `coveredThresholds = {slow_request.warning, slow_request.critical, large_response.warning, request_frequency.warning}`. |
| Frame Timing | `reproducerOnly` | [`frame_timing_reproducer_test.dart`](../test/validation/frame_timing_reproducer_test.dart) | Four stableIds pinned by hermetic reproducer: `sustained_jank` (≥3 severe frames in a 60-frame window), `jank_detected` (>15% jank frames, ≥5-frame sample), `raster_cache_thrashing` (≥15 consecutive frames of ≥20% picture-cache-count fluctuation, seeded by `previous.pictureCacheCount > 5`), and `raster_cache_growing` (≥30 consecutive frames of monotonic picture-cache-count growth). Reproducer bypasses warmup via `warmupDuration: Duration.zero`; every stableId has a synthetic `FrameStats` path plus a real `FrameTiming` integration leg via `handleTimingsForTest` so hand-written synthetic fixtures cannot encode the detector's own expected shape (anti-tautology, Tactic 9). Impeller-zero suppression (all four cache metrics zero for ≥30 frames) pinned by a dedicated `pictureCacheBytes: 1` belt-and-suspender test. v0.17.0 added a `FPS semantics` group (6 tests via the real `handleTimingsForTest` path) pinning rolling-window `actualFps` against batched delivery, window slides, 120 Hz capacity, and null-rasterFinishUs handling — stableId coverage unchanged because FPS reporting is orthogonal to jank classification. **v0.19.6** lands capture-mode plumbing + structural prerequisites for the v0.19.7 `jank_detected.warning` runtimeVerified raise. Plumbing: `FrameTimingDetector.captureMode` ctor flag (from `SleuthConfig.captureMode`) short-circuits `_isPastWarmup()` so a 4 s scenario span fits inside one screen interaction; `_emissionSeq` instance-monotonic counter combined with wall-clock micros forms `dedupIdentityMicros` (audit-gate uniqueness invariant); `extraTraceArgs` exports `observedJankCount`, `observedJankPercent`, `observedWorstFrameMs`, `bufferSize`; `FrameTimingDetector.reset()` clears buffer + ephemeral issues + warmup anchors (preserves `_emissionSeq`), auto-invoked by `SleuthController.resetCaptureState`. Detector behavioral change — Option B parallel emission: `_evaluateJank` switched from `if/else if` to `if/if`, so when both gates fire (severeCount ≥ 3 AND jankPercent > 15) BOTH stableIds emit concurrently — pre-v0.19.6 `else if` made warning-tier signal structurally unreachable on devices with ambient severeCount ≥ 3 (iPhone 12 profile mode). Schema extension: `DetectorMetadata.observedAxisReduction: 'max' | 'last'` (default `'max'` preserves existing detectors); FrameTiming opts into `'last'` for windowed-aggregate observable. `'percent'` added to `ProfileCaptureSchema.approvedUnits`. Six new reproducer tests pin captureMode short-circuit, `extraTraceArgs` shape, `_emissionSeq` monotonicity across `reset()`, reset semantics, parallel-emission overlap fixture; one existing detector test updated for parallel emission. v0.19.6 capture screen at `example/lib/demos/frame_timing_capture_screen.dart` runs three legs with rate-based UI-thread injector via `Ticker` (spins 18 ms on every Nth frame matched to leg's target jankPercent — below 0%, at 20%, above 27%), 60 Hz pre-flight, captureMode pre-flight, compose-then-stash, post-leg validator on denominator-independent jankPercent axis (extracts LAST in-span emission's `observedJankPercent`, freshness invariant rejects when `bufferSize` < 220). Helper does NOT reject on `sustained_jank.critical` co-fire (audit pipeline doesn't enforce mutual exclusion). v0.19.7 raise prerequisites: three on-device captures (iPhone 12 / iOS 17.5 / Flutter 3.41.x). v0.19.7 metadata: `perStableIdTier: {jank_detected: runtimeVerified}`, `bracketStableId: 'jank_detected'`, `bracketThreshold: 16` (NOT 15 — detector rounds + strict `> 15`, first reachable observed value is 16), `bracketUnit: 'percent'`, `bracketAtTolerance: 0.50` (at-band [16, 24]), `aboveCeilingMultiplier: 1.85` (above-band (24, 29.6]), `coveredThresholds: {jank_detected.warning}`, `observedAxisArgKey: 'observedJankPercent'`, `observedAxisReduction: 'last'`, `bracketRequireUniqueDetectedAtMicros: true`. A subsequent raise to `externallyCited` additionally requires a Flutter docs citation matching the 16.67 ms budget semantics. **v0.19.17** wires `extraTraceArgs.observedSevereCount` + `observedJankPercent` + `bufferSize` stamps and `_emissionSeq`-derived `dedupIdentityMicros` for `sustained_jank` emission. **v0.22.0** withdrew the `sustained_jank.critical` raise; bracket axis (sliding 240-frame-window severeCount) non-composable with operator-claimed K. Captures + capture screen + retainedOrphans entries removed; reproducer-tier coverage retained. |
...
| Heavy Compute | `runtimeVerified` | [`heavy_compute_reproducer_test.dart`](../test/validation/heavy_compute_reproducer_test.dart) | Reproducer pins 8 ms strict warning + 2× critical across all three emission paths. **Warning tier** (v0.18.2): canonical bracket — `threshold: 8`, `atTolerance: 0.50` (at-band [8, 12] ms), `aboveCeilingMultiplier: 1.875` (ceiling 15 ms, clears 16 ms critical). `flushTimelineNow` drives synchronous detector emission inside scenario span; `TimelineParser` reconstructs BUILD `dur` from `ph: 'B'/'E'` pairs (iOS profile-mode B/E only). **Critical tier** (v0.19.13, `additionalBrackets[0]`, first tier-stack raise): `threshold: 16`, `atTolerance: 0.60` (at-band [16, 25.6] ms — wider than warning's 0.50 as forward-compat re-record headroom; the committed 23.703 ms fits 0.50 too), `aboveCeilingMultiplier: 1.875` (ceiling 30 ms; no super-critical tier). Both brackets stamp BUILD ms into `extraTraceArgs.observedDurationMs` and declare `observedAxisArgKey: 'observedDurationMs'` so the audit cross-checks operator-Stopwatch observed against detector-side measurement (closes the wrong-magnitude gap when `magnitudeSourceEventName: ''` bypasses BUILD-derivation). Cross-spec uniqueness key `(stableId, severityLabel, argKey)` distinguishes warning + critical sharing one argKey via severity. Six on-device captures (iPhone 12 / iOS 17.5 / Flutter 3.41.x) under v0.18.1+ producer-side dedup with stable per-BUILD `detectedAt`. Warning captures: [`heavy_compute_below.json`](../test/validation/captures/heavy_compute/heavy_compute_below.json), [`heavy_compute_at.json`](../test/validation/captures/heavy_compute/heavy_compute_at.json), [`heavy_compute_above.json`](../test/validation/captures/heavy_compute/heavy_compute_above.json). Critical captures: [`heavy_compute_critical_below.json`](../test/validation/captures/heavy_compute/heavy_compute_critical_below.json), [`heavy_compute_critical_at.json`](../test/validation/captures/heavy_compute/heavy_compute_critical_at.json), [`heavy_compute_critical_above.json`](../test/validation/captures/heavy_compute/heavy_compute_critical_above.json). `coveredThresholds = {'heavy_compute.warning', 'heavy_compute.critical'}`. Critical-below leg fires `.warning` (not `.critical`); schema's name-scoped no-record check accepts. |
| Platform Channel | `runtimeVerified` | [`platform_channel_reproducer_test.dart`](../test/validation/platform_channel_reproducer_test.dart) | v0.17.4 tier-quality audit pinned the reproducer (both axes through `TimelineParser.parse()`). v0.19.4 raises `platform_channel_traffic` warning tier (frequency axis: > 20 calls / 1 s evaluation window) to `runtimeVerified` with three on-device captures (iPhone 12 / iOS 17.5 / Flutter 3.41.x) recorded under v0.19.4 producer-side dedup with stable per-window `dedupIdentityMicros = _windowStart.microsecondsSinceEpoch`. Two parser-accepted phase+name shapes covered in the reproducer — lowercase async `'b'` with `Platform Channel send ` prefix (real `debugProfilePlatformChannels` output via `TimelineTask`) and sync `'X'` with `MethodChannel` name. The capture screen sets `debugProfilePlatformChannels = true` per leg in try/finally so real `MethodChannel.invokeMethod('ping')` calls flow through the lowercase async path; channel reuses the existing `sleuth_demo_channel` registered in `example/ios/Runner/AppDelegate.swift` (handler returns nil). Uppercase sync `'B'` async-shaped events asserted non-emitting — the canonical format-boundary trap for channel observers. Parser accepts 9 channel-shape variants (6 sync-name + 3 async-prefix casings); other 7 are implicitly uncovered at this tier. Both detector axes pinned in the reproducer: (a) >20/sec frequency strict + 2× critical at 41 calls + 40 calls held at warning (critical-escalation inequality), (b) >8000µs cumulative duration per window strict (7998/8000/8001µs via sync `'X'` events). v0.19.4 also fixed cooldown semantics — sustained overload now suppresses fresh emission during the 3-cycle drain after a fire (retains prior `_lastEmittedIssue` with original dedup identity), so multi-second scenario brackets collapse to one trace record per cooldown window. `bracketAtTolerance: 0.50` (at-band [20, 30] calls/sec) absorbs iOS scheduling jitter; `aboveCeilingMultiplier: 1.95` (above-band ceiling 39 calls/sec) clears the 41-call critical-escalation boundary so the above-leg cannot ambiently bracket the critical tier. Captures: [`platform_channel_traffic_below.json`](../test/validation/captures/platform_channel/platform_channel_traffic_below.json), [`platform_channel_traffic_at.json`](../test/validation/captures/platform_channel/platform_channel_traffic_at.json), [`platform_channel_traffic_above.json`](../test/validation/captures/platform_channel/platform_channel_traffic_above.json). `coveredThresholds = {'platform_channel_traffic.warning'}`. Critical tier (41 calls/sec, `> 2× threshold`) AND the 8 ms cumulative-duration axis stay implicitly `reproducerOnly` — neither has a checked-in capture bracket. `coveredStableIds = {'platform_channel_traffic'}`. **v0.19.19** re-records the 3 capture JSONs (iPhone 12 / iOS 17.5 / Flutter 3.41.4); `PlatformChannelCaptureScreen` post-processes the wrapped JSON so `expectedMagnitude.observed` equals the detector-stamped `observedCount` (eliminates iOS coalescing divergence — detector count runs ~20 % below operator send rate). The `checkDetectorAxisInRoleBand` audit invariant enforces detector-in-role-band with disjoint at/above boundary so an above-leg detector value ≤ at-band upper (e.g. 28 in [20, 30]) is rejected even when operator-claimed magnitude lands in the schema above-band. |
| Memory Pressure | `reproducerOnly` | [`memory_pressure_reproducer_test.dart`](../test/validation/memory_pressure_reproducer_test.dart) | v0.17.4 tier-quality audit: 4 families pinned at detector entrypoints (`processHeapSample` + `recordGcCycle`). Threshold triads — `gc_pressure` (>10 cycles / 10 s sliding window = >60/min default; configurable via `SleuthConfig.gcRateThresholdPerMin`), `heap_growing` (slope >512 KB/s sustained ≥10 s), `heap_near_capacity` (>80% AND 4-of-5 samples AND correlated `heap_growing`), `native_memory_growing` (RSS-heap gap >1 MB/s sustained ≥10 s). Null-`rssBytes` (web) + zero-heap + zero-capacity null-coalesce edges asserted non-emitting. **Three upstream hops disclosed as skipped**: (1) `VmServiceClient.getMemoryUsage` repack with `null → 0` fallback (zero-coalesce edge exercised; repack itself is not); (2) `EventStreams.kGC → _onGcEvent → recordGcCycle` is the authoritative per-cycle signal and is called directly, bypassing the VM-service stream plumbing — `TimelineParser.gcEvents` over-counts sub-phase events 5–15× and is NOT used by this detector; (3) `VmServiceClient._readRssBytes() → ProcessInfo.currentRss` is the OS-level RSS collection boundary that sources `HeapSample.rssBytes` and therefore gates `native_memory_growing` — reproducer injects `rssBytes` directly so the null-rssBytes edge (web / unusual embeddings) is exercised but the `ProcessInfo` call and its try/catch are not. `coveredStableIds = {'gc_pressure', 'heap_growing', 'heap_near_capacity', 'native_memory_growing'}`. **v0.19.18** backfills `extraTraceArgs.observedSlopeBytesPerSec` (regression slope, bytes/sec) on `heap_growing` emission and declares `observedAxisArgKey: 'observedSlopeBytesPerSec'` on the canonical bracket. **v0.19.19** re-records the 3 capture JSONs (iPhone 12 / iOS 17.5 / Flutter 3.41.4); `MemoryPressureCaptureScreen` post-processes the wrapped JSON so `expectedMagnitude.observed` equals the detector slope, collapsing schema bracket-band check + per-record cross-check + `checkDetectorAxisInRoleBand` to the same authoritative value (`observedAxisTolerance` stays at schema default 0.25). New invariant enforces disjoint at/above bands at the detector axis (above-leg must strictly exceed `threshold × (1 + atTolerance)`). **v0.19.20** stamps `extraTraceArgs.observedGcEvents` + `dedupIdentityMicros` from `_gcOverageStart` on `gc_pressure` emission; same-overage emissions collapse to one trace event per episode. Lifecycle clears in `resetCaptureState` / `reset` / `dispose` and on the no-emit else-branch. 3 new reproducer tests pin axis stamping + same/distinct-overage identity. `gc_pressure` stays base reproducerOnly. |
| Stream Resource | `reproducerOnly` | [`stream_resource_reproducer_test.dart`](../test/validation/stream_resource_reproducer_test.dart) + [3 captures](../test/validation/captures/stream_resource_growth/) | Base `reproducerOnly` + `perStableIdTier{stream_resource_growth: runtimeVerified}`. Detector flags retained async resources via `getAllocationProfile` class-instance diff over a K=4 sample window, gated on `MemoryPressureDetector.isHeapGrowingActive` (default 30 s recency). Watchlist suffix-matches dart:async / dart:io / web_socket_channel classes; rxdart Subjects included only when `classRef.library.uri` contains `rxdart`. Emission requires (a) `isHeapGrowingActive` true, (b) ≥2 watchlist classes show ≥3 of 3 ascending transitions (structural precondition), (c) dominant growing class's net delta > `streamResourceMinDelta` (default 50) — single-class magnitude gate so the firing axis matches the bracketed axis (`extraTraceArgs.topGrowthDelta`). Confidence `likely`. Reproducer (5 tests) drives synthetic `AllocationProfile` responses through a fake-fetcher injection seam: co-fire emission with stable extraTraceArgs, heap_growing-off no-emit, flat profile no-emit, rxdart library-URI gate, cooldown stable identity. **`stream_resource_growth.warning` bracket**: threshold 50, unit `instances`, atTolerance 0.6 (at-band [50, 80]), aboveCeilingMultiplier 3.0 (ceiling 150 — single-tier family). `observedAxisArgKey: 'topGrowthDelta'`, `bracketRequireUniqueDetectedAtMicros: true`. Three iPhone 12 / iOS 17.5 / Flutter 3.41.4 captures: [`below`](../test/validation/captures/stream_resource_growth/below.json) / [`at`](../test/validation/captures/stream_resource_growth/at.json) / [`above`](../test/validation/captures/stream_resource_growth/above.json). Capture screen sustains 1024 KB/sec byte pressure so heap_growing re-arms inside scenario span (`markScenarioBegin → resetCaptureState` wipes the prior latch); emissions route through `pollStreamResourceAllocationProfileNowWithCapture`; JSON post-process aligns `expectedMagnitude.observed` to the detector-stamped `topGrowthDelta`. `coveredStableIds = {'stream_resource_growth'}`, `coveredThresholds = {'stream_resource_growth.warning'}`, no critical tier. |
| Repaint | `reproducerOnly` | [`repaint_reproducer_test.dart`](../test/validation/repaint_reproducer_test.dart) | Base tier `reproducerOnly` with `perStableIdTier{excessive_repaint: runtimeVerified}` (v0.21.0); `excessive_repaint_debug` and parametric `repaint_debug_<typeName>` stay base. v0.17.6 cross-harness reproducer feeds raw `List<TimelineEvent>` PAINT events through `TimelineParser.parse()` AND mounts real widget trees via `pumpWidget` for the per-widget + structural legs. All 3 families pinned with strict triads — `excessive_repaint` (VM aggregate, strict `> 30/sec`: 30/31/61), `excessive_repaint_debug` (aggregate-debug residual, `>= 30/sec`: 29/30/61), parametric `repaint_debug_<typeName>` (per-widget residual rate, `>= 30/sec`: 29/30/61). Gate B animation-owned suppression pinned with broad `expect(issues, isEmpty)` so a regression cannot leak through any of the three emission paths. Reconnect-flush pinned (cold-init false→true stages `_pendingVmWindowCount=0`). Highlights `_maxHighlightsPerType=3` cap pinned. **`excessive_repaint.warning` bracket** (v0.21.0): `threshold: 30`, `bracketUnit: 'paints'`, `bracketAtTolerance: 0.50` (at-band [30, 45] absorbs iOS 60Hz Timer-tick scheduler jitter across 32 widget types), `aboveCeilingMultiplier: 2.0` (above-band ceiling 60 sits strictly under the `> 60` critical-tier fire boundary so the above leg carries pure warning evidence). Detector stamps `extraTraceArgs.observedPaintCount` (1s window aggregate) on emission; `observedAxisArgKey: 'observedPaintCount'`, `observedAxisReduction: 'max'`. `flushPaintEvaluation()` refreshes only `lastObservedPaintCount` — peak stays bound to naturally-closed windows so the exported magnitude always matches an emitted `observedPaintCount` arg. Capture screen at `example/lib/demos/repaint_capture_screen.dart` mounts 32 distinct `_PT00..31` `CustomPaint` widget classes so the per-widget debug gate stays sub-threshold and emission flows through the VM aggregate path. Three iPhone 12 / iOS 17.5 / Flutter 3.41.4 captures: [`excessive_repaint_below.json`](../test/validation/captures/repaint/excessive_repaint_below.json), [`excessive_repaint_at.json`](../test/validation/captures/repaint/excessive_repaint_at.json), [`excessive_repaint_above.json`](../test/validation/captures/repaint/excessive_repaint_above.json). `coveredStableIds = {'excessive_repaint', 'excessive_repaint_debug'}`, `parametricFamilies = {'repaint_debug'}`, `coveredThresholds = {'excessive_repaint.warning'}`. Critical tier (>60 paints/sec), `excessive_repaint_debug`, and per-widget debug parametric stay implicitly reproducerOnly. |

### Hybrid detectors (2)

| Detector | Tier | Reproducer | Notes |
|---|---|---|---|
| Rebuild | `reproducerOnly` | [`rebuild_reproducer_test.dart`](../test/validation/rebuild_reproducer_test.dart) + [3 captures](../test/validation/captures/rebuild_detector/) | **v0.19.12 perStableIdTier raise**: `rebuild_activity.warning` raised via `perStableIdTier` backed by 3 on-device captures (iPhone 12 / iOS 17.5 / Flutter 3.41.x). Bracket: threshold 11, atTolerance 0.65 → at-band [11, 18.15], aboveCeilingMultiplier 2.7 → above-band ceiling 29.7 (strictly under critical fire `> 30`). Reduction `'max'` over multi-window in-span emissions; `observedAxisArgKey: 'observedRebuildRate'` cross-checked against detector trace records. **Baseline subtraction** required for runtimeVerified honesty: iOS profile-mode emits ~10–15 BUILDs/sec from Material framework alone (animations, theme inheritance, navigator transitions) which exceeds the default 10/sec threshold. Capture-mode operator measures ambient inline before each leg (3s idle scenario) and calls `setBaseline(int)`; detector subtracts before threshold gate AND before exposing `lastObservedRebuildRate` / `peakObservedRebuildRate`. Default baseline=0 → no-op subtraction → live monitoring unchanged. Other family `stateful_density` stays at base reproducerOnly. v0.17.6 tier-quality reproducer unchanged: cross-harness reproducer feeds raw `List<TimelineEvent>` through `TimelineParser.parse()` plus real `pumpWidget` for structural-fallback. Builder-widget 3× multiplier, `RebuildCountSource.flutterTimeline` per-type suppression, reconnect-flush, and `_maxHighlightsPerType=3` cap all pinned. `coveredStableIds = {'stateful_density', 'rebuild_activity'}`, `parametricFamilies = {'rebuild_debug'}`, `coveredThresholds = {'rebuild_activity.warning', 'rebuild_activity.critical'}`. **v0.19.21** raises `rebuild_activity.critical` via `additionalBrackets[0]`: threshold 31, atTolerance 0.65 → at-band [31, 51], aboveCeilingMultiplier 2.7 → above-ceiling 83. Same `observedAxisArgKey`, reduction `'max'`, `requireUniqueDetectedAtMicros: true`. 3 critical captures at `critical_{below,at,above}.json` (28 / 40 / 70 BUILDs/sec; v0.19.22 re-records above-leg with `targetRebuildRate: 70` for redundant in-band emissions). Capture screen exposes a tier dropdown; tier-derived scenario name + `bracketSeverityLabel`. v0.19.23 opts critical bracket into `minInBandSamples: 2`: each leg (at + above) must carry ≥2 in-band detector samples; per-leg semantics via `checkMinInBandSamplesPerSpec`. |

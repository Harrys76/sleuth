...
count: 13 family-severity pairs across 10 unique stableIds** —
...
|---|---|---|---|
| Tracked Resource | `reproducerOnly` | [`tracked_resource_reproducer_test.dart`](../test/validation/tracked_resource_reproducer_test.dart) + 3 captures under [`test/validation/captures/tracked_resource_concurrent/`](../test/validation/captures/tracked_resource_concurrent/) | Pure-Dart, opt-in. `Sleuth.trackResource(name, resource)` registers; tracker keeps `WeakReference<Object>` + Finalizer token + first-seen timestamp per registration. Token is the registration identity (allocation-unique, collision-resistant); shared `Finalizer` dispatches `_recordRelease(token)` on GC reclaim. Two stableIds, both `confirmed`: `tracked_resource_concurrent.warning` (live count > `trackedResourceMaxConcurrent`, default 5; **v0.29.0 runtimeVerified via `perStableIdTier` — bracket threshold 6 `instances`, atTolerance 0.5 (at-band [6, 9] absorbs discrete-count quantisation), aboveCeilingMultiplier 3.0 (ceiling 18), `observedAxisArgKey: 'liveInstanceCount'`, `requireUniqueDetectedAtMicros: true`, iPhone 12 / iOS 17.5 / Flutter 3.41.4 capture triad**) and `tracked_resource_long_lived.warning` (oldest instance alive past `trackedResourceLongLivedSeconds`, default 300 s; **stays reproducerOnly** — 300 s threshold too long for an on-device capture scenario window). Periodic sweep (default 10 s) prunes finalised refs + evaluates thresholds. **Capture-mode plumbing**: synchronous `flushConcurrentEvaluation()` drives a sweep inside the scenario span without waiting for the timer tick; `untrackAll(name)` drops the bucket + detaches per-ref Finalizers so capture screens can isolate back-to-back legs; `resetCaptureState()` (auto-invoked by `Sleuth.markScenarioBegin`) clears per-leg observable state + every bucket's `concurrentFirstCrossMicros`; `lastObservedLiveCount` / `peakObservedLiveCount` getters expose the value the bracket validator cross-checks against `extraTraceArgs.liveInstanceCount`. Parametric emission (`tracked_resource_concurrent:<name>`) routes the capture trace event through `PerformanceIssue.captureTraceStableId = 'tracked_resource_concurrent'` so the bracket validator's byte-exact `sleuth.issue.<id>.<severity>` filter matches every member of the family; UI cards continue to key on the parametric `stableId`. LRU cap (`trackedResourceMaxDistinctNames`, default 1000) bounds the in-memory bucket map; eviction detaches per-ref Finalizer entries so VM-side state stays bounded. `Sleuth.untrackResource` is the optional explicit decrement. Cross-isolate registration is a no-op (one controller per isolate). Primitive / record targets dropped silently via `droppedTargetsCount`. Reproducer hermetic via `simulateFinalizerForTest` seam (identity lookup → token release). `coveredStableIds = {'tracked_resource_concurrent', 'tracked_resource_long_lived'}`; `coveredThresholds = {'tracked_resource_concurrent.warning'}`. CausalGraphRule edges to `heap_growing` for both stableIds. |

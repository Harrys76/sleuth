...
count: 12 family-severity pairs across 9 unique stableIds** —
...
| Stream Resource | `reproducerOnly` | [`stream_resource_reproducer_test.dart`](../test/validation/stream_resource_reproducer_test.dart) | v0.24.0 introduces the detector. Heuristic flag for retained async resources via `getAllocationProfile` class-instance diff over a K=4 sample window, gated on `MemoryPressureDetector.isHeapGrowingActive([windowMicros])` (default 30 s recency). Watchlist suffix-matches dart:async / dart:io / web_socket_channel classes (`StreamSubscription`, `_BroadcastSubscription`, `_ControllerSubscription`, `StreamController`, `_SyncBroadcastStreamController`, `_AsyncBroadcastStreamController`, `_WebSocketImpl`, `WebSocketChannel`); rxdart `PublishSubject` / `BehaviorSubject` / `ReplaySubject` included only when `classRef.library.uri` contains `rxdart`. Emission requires (a) `isHeapGrowingActive` true, (b) ≥2 watchlist classes show ≥3 of 3 ascending transitions, (c) sum of net deltas > `streamResourceMinDelta` (default 50). Confidence `likely`. Reproducer (5 tests) drives synthetic `AllocationProfile` responses through a fake fetcher injection seam and asserts: deliberate-leak co-fire emission with stable extraTraceArgs, heap_growing-off no-emit, flat profile no-emit, rxdart library-URI gate, cooldown stable identity. **Tier raise to runtimeVerified is deferred** — current schema brackets a single observed-axis numeric magnitude (ms / bytes/sec / count) and does not yet model the multi-class delta axis this detector operates on; on-device class-instance capture infrastructure also does not yet exist. `coveredStableIds = {'stream_resource_growth'}`. Threshold-level pin (`coveredThresholds`) is intentionally omitted at this tier — the reproducerOnly anchor invariant rejects extended-claim fields, and the schema's single-axis bracketing does not yet model the multi-class delta axis this detector operates on. A future runtimeVerified raise will land both fields together. |

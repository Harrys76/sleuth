...
|---|---|---|---|
| Tracked Resource | `reproducerOnly` | [`tracked_resource_reproducer_test.dart`](../test/validation/tracked_resource_reproducer_test.dart) | Pure-Dart, opt-in. `Sleuth.trackResource(name, resource)` registers; tracker keeps `WeakReference<Object>` + Finalizer token + first-seen timestamp per registration. Token is the registration identity (allocation-unique, collision-resistant); shared `Finalizer` dispatches `_recordRelease(token)` on GC reclaim. Two stableIds, both `confirmed`: `tracked_resource_concurrent.warning` (live count > `trackedResourceMaxConcurrent`, default 5) and `tracked_resource_long_lived.warning` (oldest instance alive past `trackedResourceLongLivedSeconds`, default 300 s). Periodic sweep (default 10 s) prunes finalised refs + evaluates thresholds. LRU cap (`trackedResourceMaxDistinctNames`, default 1000) bounds the in-memory bucket map; eviction detaches per-ref Finalizer entries so VM-side state stays bounded. `Sleuth.untrackResource` is the optional explicit decrement. Cross-isolate registration is a no-op (one controller per isolate). Primitive / record targets dropped silently via `droppedTargetsCount`. Reproducer hermetic via `simulateFinalizerForTest` seam (identity lookup → token release). `coveredStableIds = {'tracked_resource_concurrent', 'tracked_resource_long_lived'}`. CausalGraphRule edges to `heap_growing` for both stableIds. |

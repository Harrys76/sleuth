...
as of v0.19.13. The detector-row tier reflects each detector's *base*
...
count: 8** across 5 multi-family detectors —
...
`HeavyCompute.heavy_compute` (single-family base raise; v0.19.13 adds critical-tier
bracket via `additionalBrackets` for the first tier-stack raise — both
`heavy_compute.warning` and `heavy_compute.critical` now in `coveredThresholds`),

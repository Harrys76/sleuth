...
| 15 | MemoryPressureDetector | growth > 512 000 B/s; capacity > 80 %; GC > 30/min; native > 1 MB/s; warmup 3 s; sustained 10 s | 🔧 ²⬆ | **Pass 2 upgraded from 🔍 → soft 🔧.** Semantics resolved (sliding 10s window × 6 = per-min extrapolation, line 233; source is `EventStreams.kGC` per-cycle, line 142). 5 GC events in 10s is plausibly within normal Dart young-gen scavenge cadence — likely too sensitive. |
...
### MemoryPressureDetector — 🔍 INVESTIGATE (GC-rate semantics)
**Current:** `gcPerMinute > 30` fires an issue, `_gcWindowDuration = Duration(seconds: 10)`.
**Leading hypothesis:** 30 GCs/min (one every 2 s) is **within normal young-generation cadence for Dart** — UNLESS the metric is extrapolated from a 10-second window, in which case "30/min" means 5/sec in the window, which DOES indicate genuine pressure.
**Falsification test:** Read the sliding-window code in `memory_pressure_detector.dart` to determine rate semantics.
**Verdict: INVESTIGATE** — either way the comment-free threshold is unreviewable.
...
**Newly added (from §7):**
- **MemoryPressureDetector GC threshold**: promote from `INVESTIGATE` to soft `TUNE` — the threshold semantics are now resolved (5 events in 10 s), but 5 GC cycles in 10 s is plausibly within normal Dart young-gen scavenge cadence. Either raise the threshold to 60–90/min, or filter the kGC stream to old-space cycles only in the controller's `_onGcEvent`. Add an inline comment either way — currently unreviewable.
...
| 🔍 INVESTIGATE | MemoryPressureDetector GC rate | 🔧 soft TUNE (resolved) | `memory_pressure_detector.dart:233` — `gcPerMinute = (windowEvents / 10s) * 60`. So `> 30/min` = "more than 5 events in last 10 s." Window source confirmed at line 142: `EventStreams.kGC` "exactly one event per completed GC cycle" (includes new-space scavenges AND old-space). 5 GC cycles in 10 s is plausibly within normal Dart young-gen scavenge cadence on any moderately-allocating UI. **Either** raise threshold to 60–90/min, **or** filter to old-space cycles in the controller's `_onGcEvent` handler. |

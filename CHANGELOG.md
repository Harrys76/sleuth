...
**Four structural detectors raised to `reproducerOnly` — third v0.16 milestone.** Single release lifts `ImageMemoryDetector`, `OpacityDetector`, `ListviewDetector`, and `GlobalKeyDetector` out of `unvalidated`. Each claim is backed by a hermetic reproducer test that pins the detector's thresholds end-to-end via the real `scanTree` path. Pure-structural detectors were chosen because their trigger paths are fully exercisable from widget tests — no profile-mode capture, VM timeline, or decoded bitmap dependency — so the reliability contract is tractable in a single PR.

### Added

- **`test/validation/image_memory_reproducer_test.dart`** — 6 tests pinning `uncached_images`: 40×40 / 50×50 / 51×51 / 100×100 boundary triad proving the 50dp small-image skip is inclusive at exactly 50dp and fires at 51dp, `ResizeImage`-wrapper suppression (documented fix is the only documented suppressor), and the "zero is NOT small" unconstrained-size policy — a bare `Image` with no enclosing `SizedBox` still fires because a 0×0 Image is almost always a missing-constraint bug, not a real tiny image. Uses a 58-byte minimal-valid 1×1 transparent PNG via `MemoryImage`; decode completion irrelevant because detector is a pure structural scan.
- **`test/validation/opacity_reproducer_test.dart`** — 5 tests pinning `opacity_zero`: exact-zero contract (`Opacity(0.0)` fires; `Opacity(0.005)` and `Opacity(0.5)` do not — near-zero is NOT zero, per the v9.1 detector docstring); `AnimatedOpacity(0.0)` settled at 0.0 fires exactly once so the inner `FadeTransition` is suppressed by the `_insideAnimatedOpacity` depth counter (AnimatedOpacity internally builds a FadeTransition — naive counting would double-report); nested `Opacity(0.0)` inside `Opacity(0.0)` produces one rollup issue whose title reflects both occurrences (count = 2).
- **`test/validation/listview_reproducer_test.dart`** — 8 tests pinning 3 of 8 stable-id families: `non_lazy_listview` (childThreshold boundary — 5 at-threshold no fire, 6 above fires, 20 well-above → critical severity — plus `ListView.builder` lazy path bypasses regardless of itemCount because `SliverChildBuilderDelegate` is NOT `SliverChildListDelegate`); `sliver_to_box_adapter_large` (6-child `Column` wrapped in `SliverToBoxAdapter` fires, 5 at-threshold does not); `sliver_fill_remaining_scrollable` (`SliverFillRemaining(hasScrollBody: false)` wrapping a scrollable fires — eager-build trap — and `hasScrollBody: true` wrapping the same scrollable does NOT, documenting the correct use). Remaining 5 families (`non_lazy_gridview`, `non_lazy_sliver_list`, `non_lazy_sliver_grid`, `sliver_to_box_adapter_shrinkwrap`, `non_lazy_list`) remain implicitly `unvalidated` — same single-family-pin precedent as v0.16.1 NetworkMonitor, called out in the ledger row.
- **`test/validation/global_key_reproducer_test.dart`** — 7 tests pinning both `GlobalKeyDetector` families. `excessive_global_keys`: 3 at-threshold no fire (strict greater-than), 4 just above fires, 10 (> 3× threshold) fires critical, and GlobalKeys on bare widgets outside any scrollable do NOT fire because the detector only counts keys while a `ListView` / `GridView` / `PageView` is on its internal scrollable stack. `global_key_recreation`: two scans with fresh `GlobalKey()` instances on the same scan root fires (`Directionality` Element survives `pumpWidget` when top-level type is stable, so min(newKeys, goneKeys) = 4 ≥ recreationThreshold = 2); two scans reusing the same GlobalKey instances held in State fields does NOT fire (identity hashes unchanged — the contract developers satisfy by storing GlobalKeys in State fields, not building them inside `build()`); first scan alone does NOT fire because `_prevKeyIds` is empty. `coveredStableIds` uses the canonical prefix form `excessive_global_keys` per the ledger's prefix convention for indexed `excessive_global_keys:<i>` emissions.

### Changed

- **`ImageMemoryDetector.validationMetadata`** — `tier: unvalidated` → `reproducerOnly`, `reproducerPath: 'test/validation/image_memory_reproducer_test.dart'`, `coveredStableIds: {'uncached_images'}`, rationale rewritten to describe what the reproducer validates.
- **`OpacityDetector.validationMetadata`** — `tier` raised to `reproducerOnly`, `reproducerPath` set, `coveredStableIds: {'opacity_zero'}`, rationale rewritten.
- **`ListviewDetector.validationMetadata`** — `tier` raised to `reproducerOnly`, `reproducerPath` set, `coveredStableIds: {'non_lazy_listview', 'sliver_to_box_adapter_large', 'sliver_fill_remaining_scrollable'}` (3 of 8), rationale rewritten with explicit call-out that the other 5 families stay `unvalidated` pending separate reproducers.
- **`GlobalKeyDetector.validationMetadata`** — `tier` raised to `reproducerOnly`, `reproducerPath` set, `coveredStableIds: {'excessive_global_keys', 'global_key_recreation'}`, rationale rewritten.
- **`doc/validation_ledger.md`** — summary line flipped from `1 / 23 at reproducerOnly, 22 / 23 at unvalidated` to `5 / 23 at reproducerOnly, 18 / 23 at unvalidated`; structural rows for ListView, Image Memory, GlobalKey, and Opacity now link their reproducer files and describe what is pinned; roadmap line added for v0.16.3; date bumped to 2026-04-21.

### Notes

- No public API surface changes; no schema changes; no dependency changes. Migration for external forks is zero — the four affected detectors' `validationMetadata` getters are still `const` and still satisfy the audit gate (now with strictly more information, not less).
- v0.16 arc remains pre-1.0 validation infrastructure, so patch bump (0.16.2 → 0.16.3) fits semver.
- `fvm flutter analyze` clean, `fvm flutter test` all green, `fvm flutter pub publish --dry-run` clean. Test count 2,391 → 2,417 (+6 `image_memory_reproducer` + 5 `opacity_reproducer` + 8 `listview_reproducer` + 7 `global_key_reproducer`). The detector-metadata audit gate now counts 5 `reproducerOnly` entries and enforces that each reproducer file exists, exercises the correct detector class via AST-based gating, and declares non-empty `coveredStableIds`.

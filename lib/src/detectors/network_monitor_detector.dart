...
class NetworkMonitorDetector extends BaseDetector
  ...
  DetectorMetadata get validationMetadata => const DetectorMetadata(
        ...
        rationale: 'Hermetic reproducer: direct `processRecord` boundary '
            ...
            'v0.16.5 second staged `externallyCited` raise REVERTED after '
            '`/advanced-adversarial-review` triangulation converged on '
            'two blockers: (1) the NN/g "Response Times: The 3 Important '
            'Limits" 1.0 s boundary is a UI direct-manipulation feedback '
            'guideline, not a generic HTTP latency threshold — the '
            'detector emits `slow_request` for any uncancelled request '
            'regardless of whether it blocks user interaction, so the '
            'citation does not substantiate the detector contract; (2) '
            ...
            'trace record with the detector-measured duration, AND extend '
            '`ProfileCaptureSchema.validateBracket` to require that '
            'record inside the scenario window. Critical tier (3000 ms) '
            'stays `reproducerOnly` — not covered by any external '
            'citation. `slow_request` `coveredStableIds` entry preserved. '
            'Other issue families (large_response, request_frequency, '
            'http_error_spike, high_frequency_same_path) remain '
            'implicitly unvalidated — see `coveredStableIds`.',

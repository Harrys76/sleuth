...
**v0.33.0** (current) — MCP wire-shape lock. `doc/mcp_schema.json` + `doc/mcp_schema.md` ship in the pub archive; `test/validation/mcp_schema_audit_test.dart` enforces bidirectional key match for the 7 `ext.sleuth.*` handlers. `kSleuthPackageVersion` 0.32.0 → 0.33.0; envelope `schemaVersion` stays at `1` (additive release). `routeHealth` match-route now wraps `RouteSession` under `{route: <session>}` for parity with the absent-route `{routes: [...]}` shape. Sidecar `sleuth_mcp` v0.3.0 pins to 0.33.0; `acceptedPriorLineages = {0.32}` one-cycle fallback for 0.32.x apps mid-upgrade; sidecar's `get_route_health` normalizes the v0.32 inline shape. Bridge layer owns version-skew refusal — `RealVmBridge.versionSkewValidator` funnels every connect / reconnect / refresh through one chokepoint and fails closed on missing or non-String `packageVersion`. Deep `snapshot` + `routeHealth` fields delegate to model `toJson()`; sidecar tool-layer audit deferred to v0.4.0.

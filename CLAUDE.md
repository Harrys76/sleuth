...
**v0.18.2** (current) — Second `runtimeVerified` raise: `HeavyComputeDetector.heavy_compute` (warning tier, 8 ms threshold). First vmOnly raise enabled by v0.18.1's `Sleuth.flushTimelineNow()`. Distribution: **21/23 `reproducerOnly`, 2/23 `runtimeVerified`, 0/23 `unvalidated`**. Three on-device captures (iPhone 12 / iOS 17.5 / Flutter 3.41.x) recorded under producer-side dedup with stable per-BUILD `detectedAt`. v0.18.2 also extends `TimelineParser` to reconstruct `BUILD` `dur` from `ph: 'B'`/'E'` pairs (iOS profile mode emits B/E only, no X-form BUILDs — without this fix HeavyComputeDetector saw zero BUILDs from real iOS captures). HeavyCompute critical tier (16 ms) + 4 NetworkMonitor non-`slow_request` families (`large_response`, `request_frequency`, `http_error_spike`, `high_frequency_same_path`) remain implicitly `unvalidated` because `DetectorMetadata` carries one tier per detector; honest per-family-tier representation deferred to a future metadata extension.

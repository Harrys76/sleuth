...
- **v0.13.1** (current) — Dark/light mode toggle, design system tokens, header optimization, false-positive fix. New: in-overlay theme toggle icon, `Sleuth.updateTheme()` static API, `SleuthController.themeOverride` ValueListenable, `didChangePlatformBrightness` system reactivity (gated to auto-detect mode), 9 typography tokens (`fontXxs`–`fontDisplay`) and 7 border radius tokens (`radiusSm`–`radiusFull`) on `SleuthThemeData` with full migration across all 8 overlay UI files (~119 font sizes, ~50 border radii). Changed: Guide icon moved from header to footer, highlight toggle shrunk to 24px, footer Semantics consistency, `gripDots` light theme override, color coupling documentation. Fixed: `stateful_density` false positive from Sleuth overlay widgets (8 names added to `_frameworkWidgetNames` exclusion set). Three adversarial reviews (general ×2, theme performance + design system), all findings fixed. Test count: 1,985 → 1,988.

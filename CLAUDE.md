...
**v0.30.0** (current) — `tracked_resource_long_lived.warning` runtimeVerified via `additionalBrackets[0]` (axis `oldestInstanceAgeSeconds`, unit `seconds`; threshold 300, atTolerance 0.5, aboveCeilingMultiplier 3.0). Three iPhone 12 / iOS 17.5 / Flutter 3.41.4 captures with real-time waits at the 300 s default. Detector overwrites `longLivedFirstCrossMicros` each sweep (was `??=`) so an overshoot emits per sweep with monotonically-increasing age; `observedAxisReduction: 'max'` picks the leg-end value. Adds `lastObservedAgeSecondsFor(name)` / `peakObservedAgeSecondsFor(name)` per-name observables.

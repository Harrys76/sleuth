...
fvm flutter test                    # Run all tests (~1,823 tests, ~20s)
...
- **v0.11.1** (current) — Pillar 5 Part 2 demo quality enhancements: Before/After toggle in `DemoScaffold` (now a `StatefulWidget`), working fixed-pattern bodies for all 23 existing demos, live `MetricsBar`/`MetricChip` wired into 7 demos, reproduction instructions (`❌ BAD / ✅ FIX / ▶ action`) on every demo, and two new realistic multi-detector demos: E-Commerce Product Page (hero carousel + price `AnimatedBuilder` + `IntrinsicHeight` size row + 200-review list + 4 `GlobalKey`s) and Chat App (tabbed conversations + `AutomaticKeepAliveClientMixin` + 40ms platform-channel typing poll + top-level `setState` on message arrival). Demo count: 23 → 25. **Three adversarial review rounds, 18 findings resolved.** Round 4 (9 findings): FAB double-action gate (`_isFixed` sync via `onToggle`) in three demos, E-Commerce hero carousel forced into tree via `Row` (was invisible to `ImageMemoryDetector` behind `ListView.builder` cacheExtent), bounded-viewport fix for the "paginated" reviews list (was still realizing all 200 items under `shrinkWrap`), `HttpClient` try/finally leak fix in network stress, bounded message growth in chat demo. Round 5 (4 findings): in-build counter reset race in `non_lazy_list_demo`, cacheWidth/avatar size text mismatches, missing `▶` marker. Round 6 (5 findings, demo↔detector alignment): `global_key_demo` 15→25 keys (above `threshold=20`), `non_lazy_list_demo` 40→60 children (above `childThreshold=50`), `animated_builder_demo` 6→12 progress bars with richer Row structure (~135 widgets > `minSubtreeSize=50`), `combined_analytics_dashboard_demo` 25→60 tiles with in-build `GlobalKey` allocation under outer-`AnimatedBuilder` wrap forcing per-tick rebuilds (fires both ListView and GlobalKey **recreation** branches), `nested_scroll_demo` 30→60 children with added outer scrollable wrapper (original demo had no nesting at all). Plus a `KeepAliveDetector` false-positive bug fix uncovered during the chat demo migration: detector now reads `KeepAliveParentDataMixin` from the render-object parent data instead of the stale `widget.keepAlive` field that `AutomaticKeepAlive` updates via `applyWidgetOutOfTurn`. Test count: 1,823 → 1,825. See `doc/spec_v11_pillar5_part2.md` and `doc/spec_v11.md`.
...
- v11 Pillar 5 Part 2: 7/7 milestones shipped ✅ (M8–M14, three adversarial review rounds + KeepAliveDetector bug fix). See `doc/spec_v11_pillar5_part2.md`.
- 1,825 tests, 0 analysis issues

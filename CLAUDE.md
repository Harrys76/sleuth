...
**v0.33.0** (current) — MCP wire-shape lock. `doc/mcp_schema.json` (structured contract) + `doc/mcp_schema.md` (human view) ship in the pub archive; `test/validation/mcp_schema_audit_test.dart` enforces bidirectional key match for the 7 `ext.sleuth.*` handlers. `kSleuthPackageVersion` 0.32.0 → 0.33.0. `kMcpEnvelopeSchemaVersion` = `1` — this release establishes the canonical wire contract; `routeHealth` match-shape was rationalized from inline `RouteSession.toJson()` to a `{route: <session>}` wrapper for parser consistency with the absent-route `{routes: [...]}` shape (a shape change, not envelope-version-additive). Future field rename / removal / type change will bump `kMcpEnvelopeSchemaVersion`. Companion sidecar `sleuth_mcp` v0.3.0 pins to 0.33.0 with a one-cycle `acceptedPriorLineages` fallback for 0.32.x apps mid-upgrade; the sidecar's `get_route_health` tool layer normalizes the legacy inline shape into the canonical wrapper. Sidecar bridge layer owns version-skew refusal (RealVmBridge accepts a `versionSkewValidator` so every connect / reconnect path funnels through one chokepoint and fails closed on missing or non-String `packageVersion`). Deep `snapshot` + `routeHealth` data fields delegate to model `toJson()`; sidecar tool-layer audit deferred to v0.34.0.
...
- **v0.33.0** — MCP wire-shape lock. `doc/mcp_schema.json` + `.md` ship in pub archive; `test/validation/mcp_schema_audit_test.dart` bidirectional key audit for 7 `ext.sleuth.*` handlers. `kSleuthPackageVersion` 0.32.0 → 0.33.0. `routeHealth` match-route shape changed from inline `RouteSession.toJson()` to `{route: <session>}` wrapper for parser parity with `{routes: [...]}` absent-route shape; envelope `schemaVersion` stays at `1` since this release defines the canonical contract. Sidecar v0.3.0 with `acceptedPriorLineages` transition fallback + bridge-layer `versionSkewValidator` chokepoint (fail-closed on missing/non-String `packageVersion`); `get_route_health` tool layer normalizes legacy v0.32 inline responses.

...
- **Reproducer path convention**: `reproducerPath` points at `test/validation/<d>_reproducer_test.dart` when a purpose-written reproducer exists (v0.16.3, v0.17.1 batches). It points at `test/detectors/<d>_detector_test.dart` when the existing detector unit tests already exercise the emission path and satisfy the audit gate (v0.17.2 batch). Both pass `isPathInsideRepo`. New tier raises pick whichever fits the detector's test shape.
- **Known audit methodology gap — underscore-parametric families**: the `checkReproducerFile` audit prefix convention uses `:` as the separator (`value == id || value.startsWith('$id:')`). Emitted stableIds using `_` as separator — e.g. `repaint_debug_<typeName>`, `rebuild_debug_<typeName>` — cannot be declared as a family at detector scope. Concrete instances may be test-exercised (e.g. `repaint_debug_CustomPaint`) but cannot ratchet through the audit. Two detectors ship with documented narrowings disclosing this (`RepaintDetector`, `RebuildDetector`). Fix path for a future methodology update: extend `coveredStableIds` / matcher to support explicit family patterns beyond `:`, then block detector-level tier raises while any emitted family is unrepresentable.

...
**v0.19.18** (current) — `MemoryPressureDetector.heap_growing` emission stamps `extraTraceArgs.observedSlopeBytesPerSec` + canonical bracket declares `observedAxisArgKey`. **No detector raises**; distribution unchanged. Per-detector audit now enforces both presence (declaration via `checkRuntimeVerifiedRequiresObservedAxisArgKey`) AND fidelity (captures actually carry the arg via `checkCapturesCarryObservedAxisArg`). Grandfathered brackets — MemoryPressure `heap_growing.warning` and PlatformChannel `platform_channel_traffic.warning` — exempt via typed `legacyObservedAxisAllowlist` with `consumeBy=0.21.0` deadline enforced by `checkLegacyObservedAxisManifest` (same lifecycle shape as `retainedOrphans`). Backward-compat skip semantics pinned by a regression test in `profile_capture_schema_test.dart`. `BracketSpec`/`DetectorMetadata` docstrings name the fidelity contract. 2,964 tests passing.

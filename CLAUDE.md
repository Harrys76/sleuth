...
fvm flutter test                    # Run all tests (~3,087 tests, ~45s)
...
**v0.35.0** (current) — MCP snapshot projection + pagination. `ext.sleuth.snapshot` accepts optional `sections` (comma-separated `SnapshotSection` keys), `maxIssueCount` (top-N already-ranked issues), `maxRouteCount` (N most-recent routes by `startedAt`) so long-session snapshots stay under the MCP client token cap. Backward-compatible — no args = full payload. Typed errors `arg_invalid_section` / `arg_invalid_int` / `arg_pagination_unused`. When projected, the envelope gains `_projectedSections` (alphabetically sorted), `_projectionLimits`, `_projectionApplied` (`by_app` | `by_sidecar_fallback`). New `SnapshotSection` enum (`lib/src/models/snapshot_sections.dart`, exported via barrel) = single source of truth for 14 projectable keys; drift test cross-checks it against `SessionSnapshot.toJson` + `doc/mcp_schema.json`. Metadata keys (`schemaVersion`, `exportedAt`, `packageVersion`, `isVmConnected`, `isDebugMode`, `suppressedCount`) always serialize. `kSleuthPackageVersion` 0.34.0 → 0.35.0; envelope `schemaVersion` stays at `1`. `checkSnapshotCapturesMatchSchema` gates per-section required checks on `_projectedSections`. Sidecar `sleuth_mcp` v0.6.0 pins to 0.35.0: `get_snapshot` forwards the projection args + adds `diskHandoff: bool` (writes envelope to `Directory.systemTemp`, mode 0600, crypto-random name; returns `{path, sizeBytes, sha256}` + projection metadata; cleaned on detach/shutdown/age-30min-sweep; stamps `_projectionApplied: by_sidecar_fallback` when the app predates projection). `compare_snapshots` rejects differently-projected inputs (`arg_section_mismatch`, Set-compared); `evaluateBudgets` returns `arg_missing_required_section` on a projected snapshot missing a needed section. `acceptedPriorLineages = {0.34}`. Sidecar v0.6.1 (lib unchanged) adds iOS attach mDNS-staleness auto-recovery: on a dead-port `bridge.connect` failure (`Connection reset` — iproxy resets when the device refuses the channel — or `Connection refused`) the attach retries once, re-resolving Bonjour with the dead port excluded (`IosAttacher.excludePorts`) so selection lands on the live service, which announces a different port within the staleness window. Each `xcrun devicectl` call is bounded by `devicectlTimeout` (20s) so a disconnected device can't hang the attach (and the `attach_in_progress` mutex) indefinitely; `attachBudget` (90s) gates whether recovery is attempted. Sidecar v0.6.2 (lib unchanged) extends this to the `ios_ambiguous_pairings` case: when a stale + a fresh service coexist with distinct authCodes, `attachViaIos` iterates the announced authCodes (`authOverride` per candidate) and keeps the one that connects (dead port resets); a user `authOverride` still pins one (no iteration); all-dead or 3+ records fall back to `ios_vmservice_busy`.

**v0.34.0** — MCP snapshot deep-shape lock + sidecar tool-layer audit. `kSleuthPackageVersion` 0.33.0 → 0.34.0. `doc/mcp_schema.{json,md}` codify nested shapes for `recurrenceTrends`, `sessionSummary`, `routeSessions[]`, derived from 6 on-device captures. Sidecar v0.4.0–v0.5.1: tool-schema lock + `attach_app(udid:, bundle:)` iOS-direct pipeline with `forceRelaunch`, bounded `bridge.connect`/`getVM` timeouts, `PidfileLockGuard` orphan reaping, wireless error pins.

**v0.33.0** — MCP wire-shape lock. `doc/mcp_schema.json` + `doc/mcp_schema.md` ship in the pub archive; `test/validation/mcp_schema_audit_test.dart` enforces bidirectional key match for the 7 `ext.sleuth.*` handlers. `kSleuthPackageVersion` 0.32.0 → 0.33.0; envelope `schemaVersion` stays at `1` (additive release). `routeHealth` match-route wraps `RouteSession` under `{route: <session>}`. Sidecar v0.3.0 with `acceptedPriorLineages = {0.32}` + bridge-layer `versionSkewValidator` chokepoint.

**v0.30.0** — `tracked_resource_long_lived.warning` runtimeVerified via `additionalBrackets[0]` (threshold 300 s, iPhone 12 capture triad with real-time waits at the 300 s default).
...
**v0.29.0** — `tracked_resource_concurrent.warning` raised to runtimeVerified via `perStableIdTier`. Bracket threshold 6, atTolerance 0.5, aboveCeilingMultiplier 3.0. New `PerformanceIssue.captureTraceStableId` routes parametric emissions through bare family for byte-exact bracket matching. Detector ships `flushConcurrentEvaluation()`, `untrackAll(name)`, `resetCaptureState()`, per-name peak/last getters.
...
**Distribution (current)**: 18/20 reproducerOnly base + 2/20 runtimeVerified base, 15 effective runtimeVerified family-severity pairs across 12 unique stableIds (slow_request {warning + critical}, large_response.warning, request_frequency.warning, heap_growing.warning, platform_channel_traffic.warning, jank_detected.warning, rebuild_activity {warning + critical}, heavy_compute {warning + critical}, excessive_repaint.warning, stream_resource_growth.warning, tracked_resource_concurrent.warning, tracked_resource_long_lived.warning).
...
- **v0.30.0** — `tracked_resource_long_lived.warning` runtimeVerified via `additionalBrackets[0]` (threshold 300 s, atTolerance 0.5, aboveCeilingMultiplier 3.0; iPhone 12 capture triad with real-time waits at production threshold). Detector overwrites `longLivedFirstCrossMicros` each sweep so overshoot emits per sweep with monotone age. `lastObservedAgeSecondsFor` / `peakObservedAgeSecondsFor` per-name observables.
- **v0.29.1** — `IssueEncyclopediaPage` learn-more now resolves parametric / dynamic-suffix stableIds via new `IssueExplanationBuilder.canonicalId(String)` helper. Fixes silent no-scroll for `tracked_resource_concurrent:<name>`, `excessive_keep_alive:<i>`, `repaint_debug_<typeName>`, etc.

...
**v0.19.15** (current) — Audit invariant hardening. No detector raises; schemaVersion stays v1. Three new audit checks plus stricter `parseFile`: (1) `checkCanonicalCoveredThresholdBacking` — every dotted entry in canonical `coveredThresholds` must be backed by a matching `(stableId, severityLabel)` across canonical + `additionalBrackets[*]`. (2) `checkBracketBoundsSanity` — `observedAxisTolerance ∈ (0, 0.25]`, `atTolerance ∈ [0, 1.0]`, `aboveCeilingMultiplier ∈ (1, 5]`, `observedAxisReduction ∈ {max, last}`. (3) `checkCapturePathPerDirectoryNamingUniformity` — every committed capture in a directory must use one shape (basename-exact or a common scenario-prefix); non-conforming scenarios fail. `ProfileCaptureSchema.parseFile` (and `_parseOrThrowWithLabel`) now cross-checks `metadata.scenario` against the file basename — accepts `scenario == basename` or `scenario.endsWith('_<basename>')`; only escape is parent dir `_fixtures`. `parse(List<int>)` byte-only entry unchanged. 24 new tests including production fixtures for NetworkMonitor + HeavyCompute + RebuildDetector. 2,948 tests passing.

**Distribution (current)**: 20/23 reproducerOnly base, 9/23 effective runtimeVerified families (slow_request {warning + critical}, large_response, request_frequency, heap_growing, platform_channel_traffic, jank_detected, rebuild_activity, heavy_compute {warning + critical}). Every runtimeVerified bracket declares `observedAxisArgKey`.

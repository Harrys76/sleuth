...
**v0.19.16** (current) — Audit + schema polish. No detector raises; no public API changes; schemaVersion stays v1. `_validateScenarioMatchesPath` routes through `package:path` (`p.basename` + `p.basenameWithoutExtension`) — fixes silent over-strict behavior on trailing-separator parent paths and aligns with the audit-side helper; `path` promoted from `dev_dependencies` to `dependencies`. Operator-typo `.json`-only files rejected via dotfile semantics + strict suffix rule. `checkBracketBoundsSanity` tightens two bounds: `atTolerance` from `[0, 1.0]` to `[0, 0.65]` (production max 0.60), `aboveCeilingMultiplier` from `(1, 5]` to `(1, 3]` (production max 2.7). 2,949 tests passing.

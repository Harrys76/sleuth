...
- **v0.13.1** (current) — Dark/light mode toggle, header optimization, false-positive fix. New: in-overlay theme toggle icon, `Sleuth.updateTheme()` static API, `SleuthController.themeOverride` ValueListenable, `didChangePlatformBrightness` system reactivity (gated to auto-detect mode). Changed: Guide icon moved from header to footer, highlight toggle shrunk to 24px, footer Semantics consistency. Fixed: `stateful_density` false positive from Sleuth overlay widgets (8 names added to `_frameworkWidgetNames` exclusion set). Two adversarial reviews, one finding fixed. Test count: 1,985 → 1,988.

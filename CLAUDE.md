...
**v0.34.0** (current) — MCP snapshot deep-shape lock + sidecar tool-layer audit. `kSleuthPackageVersion` 0.33.0 → 0.34.0; envelope `schemaVersion` stays at `1`. `doc/mcp_schema.{json,md}` now codify nested shapes for `recurrenceTrends.<stableId>` (required: `trend`, `totalOccurrences`, `totalObserved`, `lastSeenCycle`; optional `severityStats`), `sessionSummary` (required: `topIssues`, `frameHistogram`, `detectorHitRates`; optional `memoryTrendSummary`, `causalEdges`), and `routeSessions[]` item shape (required: `routeName`, `tabVisitIndex`, `startedAt`, `healthScore`, `durationSeconds`, `scanCycles`, `frameStats`, `issueCount`/`criticalCount`/`warningCount`, `issues`; conditional `scaffoldHashKey`, `endedAt`, `hotReloadGeneration`, `rebuildCountsByType`, `totalRebuilds`, `frameStats.{p50,p95,p99}`). Derived from 6 on-device captures (real iPhone, iOS 17.5) under `test/validation/captures/mcp_snapshots/` — see `doc/mcp_schema_derivation.md`. New `recordRecurrenceForTest` + `seedRouteHistoryForTest` `@visibleForTesting` seams + `checkSnapshotCapturesMatchSchema` invariant. `widgetHeatMap` remains the only `auditUnreachable` entry. Sidecar `sleuth_mcp` v0.4.0 pins to 0.34.0; new `packages/sleuth_mcp/doc/mcp_tool_schema.{json,md}` + `test/schema/mcp_tool_schema_audit_test.dart` lock tool-call return shapes for 9 first-class tools (success `data:` + error `errors:`) and document shims for 4 passthroughs (`severity_filter` on `get_issues`, `lineage_route_wrapper` on `get_route_health`). Mirror-parity audit asserts the tool schema is sidecar-only. `acceptedPriorLineages = {0.33}` one-cycle fallback for 0.33.x apps mid-upgrade. Sidecar v0.4.1 adds `sleuth_mcp attach-ios <udid>` — one-command replacement for the six-step manual iOS-on-real-device flow (xcrun devicectl launch + Bonjour resolve + iproxy child held for the session); no wire-shape change. Sidecar v0.5.0 folds the same pipeline into `attach_app(udid:, bundle:)` — single MCP call drives launch → Bonjour → iproxy → bridge.connect with 13 typed errors. Sidecar v0.5.1 hardens the attach pipeline: `PidfileLockGuard.registerSpawn` reaps orphan iproxy on lock timeout; new `forceRelaunch` arg + internal auto-retry on probe-then-`Connection refused` (stale mDNS recovery); `bridge.connect` 10s end-to-end + `getVM()` 3s per-RPC timeouts close the half-open-VM wedge; `IosAttachException` records `state: error` + typed `lastError` before rethrow; `mapBridgeConnectErrorToLastError` adds wireless pins (`Operation not permitted`, `Network is unreachable`, `Failed host lookup`); routing args trimmed at boundary; `_cleanup` iOS-teardown budget 3s → 6s.

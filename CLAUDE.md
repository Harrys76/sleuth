...
**v0.19.14** (current) — `NetworkMonitorDetector.slow_request.critical` raised to **runtimeVerified** via `additionalBrackets[2]`. Second tier-stack raise. Detector stamps `extraTraceArgs.observedDurationMs` (worst record's `durationMs`); both brackets declare `observedAxisArgKey: 'observedDurationMs'` + `observedAxisTolerance: 0.10` so the audit cross-checks operator-Stopwatch against detector-side measurement (loopback paths agree to the millisecond). Critical: threshold 3000 ms, atTolerance 0.40 (at-band [3000, 4200]), aboveCeilingMultiplier 2.0 (ceiling 6000 ms). Cross-spec uniqueness tuple `(stableId, severityLabel, argKey)` distinguishes the warning + critical pair sharing one argKey. Capture screen: severity-tier sub-dropdown on slow_request mode + `_slowRequestLegSpec` resolver retunes target ms / scenario / file name per tier; `_endScenarioOnce` helper makes scenario closing idempotent across dispose + per-mode catch sites; `SafeArea` wraps body. NetworkMonitor anchor covers both brackets + canonical argKey + 4-tuple uniqueness. Reproducer: 5 critical-band cases + Layer 2 end-to-end critical case at 3500 ms. Schema-edge test pins 4200/4200.001 seam at atTolerance=0.40. 6 on-device captures (iPhone 12 / iOS 17.5 / Flutter 3.41.x). `fvm flutter analyze` clean.

**Distribution (current)**: 20/23 reproducerOnly base, 9/23 effective runtimeVerified families (slow_request {warning + critical}, large_response, request_frequency, heap_growing, platform_channel_traffic, jank_detected, rebuild_activity, heavy_compute {warning + critical}). Every runtimeVerified bracket now declares `observedAxisArgKey`.
